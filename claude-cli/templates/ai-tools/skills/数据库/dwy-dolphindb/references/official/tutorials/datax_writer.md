---
source_url: https://docs.dolphindb.cn/zh/tutorials/datax_writer.html
fetched_at: 2026-05-19T09:48:12Z
category: tutorials
title: 基于 DataX 的 DolphinDB 数据导入工具
sha1: 9772ba0245cdc1f6a081dde15a432e081fd179a9
---
...
DataX 是阿里巴巴集团内被广泛使用的离线数据同步工具/平台，实现包括 MySQL、Oracle、SqlServer、PostgreSQL、HDFS、Hive、ADS、HBase、TableStore(OTS)、MaxCompute(ODPS)、DRDS 等各种异构数据源之间高效的数据同步功能，[DataX 已支持的数据源](https://github.com/alibaba/DataX/blob/master/README.md#support-data-channels)。
...
将[源码](https://gitee.com/dolphindb/datax-writer)的 *./dist* 目录下的整个 dolphindbwriter 文件夹拷贝到 dataX/plugin/writer 目录下，即可以使用。
...
- **DolphinDB 写插件配置项**
  ...
  | batchSize | 否 | INT | 100,000 | datax 每次写入 DolphinDB 的批次记录数。 |
