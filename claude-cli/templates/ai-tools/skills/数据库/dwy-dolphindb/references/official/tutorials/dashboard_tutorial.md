---
source_url: https://docs.dolphindb.cn/zh/tutorials/dashboard_tutorial.html
fetched_at: 2026-05-19T08:59:53Z
category: tutorials
title: 数据面板
sha1: d4be71498baf3b03064398e0b01e804f9960a0b4
---
...
数据面板自 2.00.11 开始为内置的 Web 组件，用户在升级到此版本后无需额外部署。
...
分享功能：可通过链接的方式将面板分享给其他用户。选中面板后点击批量分享，即可获得面板对应的链接。将链接分享给集群中的其他用户，其他用户访问链接后需要先登录自己账号，然后即可进入预览模式，不可编辑。
...
数据面板提供了柱状图、折线图、饼图、表格、K线、混合图、订单图、富文本、描述表、编辑器、仪表盘、雷达图、变量、散点图、热力图、曲面图等供用户选择，其中柱状图、折线图、多源图、散点图、混合图支持进行阈值设置。可以通过”阈值类型“选项将阈值设置为百分比或绝对值两种类型，其中百分比是指相对于该轴当前的最大值。展示类型包括以下三种选项：
...
| 标签列配置 | 选择标签列之后，可以根据数据源得到所有标签信息，配置单个标签-值的特有属性 |
