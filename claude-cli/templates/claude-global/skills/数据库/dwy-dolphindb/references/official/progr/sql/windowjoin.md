---
source_url: https://docs.dolphindb.cn/zh/progr/sql/windowjoin.html
fetched_at: 2026-05-19T09:04:04Z
category: progr
title: window join
sha1: a3fc97557d1e6c5bffd7bdd6d71e3f8dd2d9e494
---
...
- **leftTable** 和 **rightTable** 是连接的表。
  ...
  *rightTable* 不能是分布式表。
- **window**一个整型数据对或 DURATION 数据对，表示以左表中最后一个连接列的值 t
...
当 `window = w1:w2` 时，左表中每条记录的最后一个连接列的值记为
`t`，在右表中查找满足以下条件的记录：
...
- 如果右表没有与 t+w1（窗口左边界）匹配的值， `wj` 的窗口左边界值为空，而
  `pwj` 会选择 (t+w1) 前的最后一个值，将其并入窗口。
- 如果右表有多个与 t+w1（窗口左边界）匹配的值时， `wj`
  的窗口会包含所有重复的值，而 `pwj` 只包含最后一行。
...
[firstNot](https://docs.dolphindb.cn/zh/funcs/f/firstNot.html), [lastNot](https://docs.dolphindb.cn/zh/funcs/l/lastNot.html)

注：

在 window join 中，当 *aggs* 中使用 atImax 和 atImin
