---
source_url: https://docs.dolphindb.cn/zh/funcs/g/getClusterChunksStatus.html
fetched_at: 2026-05-19T09:23:01Z
category: funcs
title: getClusterChunksStatus
sha1: 77e379ab38a077cdc714aef25916404f0f67aa15
---
...
- replicas：副本的分布信息，每个副本的格式为
  `nodeAlias:replicaVersion:corrupted:inResolution:timestamp:volumeID`，多个副本之间使用逗号分隔。
- replicaCount：副本数
...
(1) 不能追加或更新数据，并只能通过调用 `drop` 类函数进行删除，且遵从事务的原则。（注意：存储在 s3
的分区不支持事务）。

(2) 不能对其进行 recovery, rebalance 以及 TSDB 引擎的 level file
合并操作。
