---
source_url: https://docs.dolphindb.cn/zh/tutorials/cluster_monitor.html
fetched_at: 2026-05-19T09:07:12Z
category: tutorials
title: 集群运维监控
sha1: e56d0971eb45ddc26708e7475f4062159e453faf
---
...
1. 在上述监控方案中，Prometheus 负责从数据源定时抓取数据。安装 [Prometheus Server](https://prometheus.io/download/)，下载对应版本的软件包：

     
   ![](images/cluster_monitor/2-3.png)
2. 将其拖拽到服务器上解压，可看到如下目录结构：
   ...
   ```
3. 在浏览器地址栏输入 `http://IP:Port` 可进入 prometheus UI 界面：

   ![](images/cluster_monitor/2-5.png)  
...
4. 编辑 prometheus.yml 并在 `scrape_configs` 模块下添加 NodeExporter 的端口信息，用于 Prometheus 定时抓取 NodeExporter 中的数据:
   ...
   ```
5. 对于 DolphinDB 集群的监控，不论是高可用集群还是普通集群，亦或是单节点，需将所有节点的端口信息添加到 `scrape_configs` 中，以3个控制节点、3个代理节点、3个数据节点的高可用集群为例，在 prometheus.yml 中的 `scrape_configs` 模块下添加以下内容:
...
6. 配置完数据源后，必须重启 Prometheus 服务。重启 Prometheus 服务后，在 Prometheus UI 界面输入 `up` 命令，检查配置是否生效【1表示正常；0表示未生效，一般情形下是由于数据源未启动导致的，可检查端口是否被占用】：
...
![](images/cluster_monitor/2-15.png)  
...
- 1 为数据源，选择 `Prometheus`；
- 2 为查询方式，建议选择 `code`；
- 3 为查询语句，输入要查询的内容，这里不仅支持简单指标查询，也支持指标之间的运算；
- 4 为时间范围；
- 5 为展示方式，有 `Table` 和 `Graph` 两种选择；
- 6 为运行查询，测试查询结果是否符合预期；
- 7 为图表的标题；
- 8 为保存按钮。
...
![](images/cluster_monitor/2-17.png)  
...
- 5 为保存按钮。点击 `Save &test` 出现如下提示，表明成功连接到 DolphinDB：

![](images/cluster_monitor/2-22.png)  

后面就可以使用配置好的数据源，通过编写脚本对 DolphinDB 集群进行监控。如下所示：

![](images/cluster_monitor/2-23.png)  

在 `Data source` 里选择配置好的 `DolphinDB-test`，之后会进入一个脚本编辑界面，如下图所示：

![](images/cluster_monitor/2-24.png)  
...
#服务器每块网卡接收速率(MB/s)
irate(node_network_receive_bytes_total[1m])/1024/1024
```
...
dolphindb-datasource 对应的文件：[DolphinDB高可用集群监控.json](script/cluster_monitor/high_available_cluster_monitor.json)

dolphindb-datasource-next 对应的文件：[DolphinDB高可用集群监控(next).json](script/cluster_monitor/high_available_cluster_monitor_next.json)

服务器监控：[服务器监控.json](script/cluster_monitor/server_monitor.json)
...
1. 使用邮件告警功能，需修改 Grafana 的配置文件中的`[smtp], [plugin.grafana-images/cluster_monitor-renderer], [unified_alerting.screenshots]`。进入 defaults.ini (默认路径为/grafana/conf/defaults.ini)，找到 `[smpt]`选项，做如下所示更改：

   ![](images/cluster_monitor/3-4.png)
2. 将 `enabled` 修改为 true，表示开启邮件通道。`host` 配置的是告警邮箱所在的服务器，例如，若以163邮箱作为告警发件邮箱，则 `host` 应为 `smtp.163.com:465`。`user` 是告警的发件邮箱，`password` 处需要填写授权码，而不是 `user` 的登录密码，内网邮箱一般只需要密码即可，请按实际情况填写。
3. 然后修改 `[plugin.grafana-images/cluster_monitor-renderer]` 选项下的 `rendering_language`，将其赋值为 zh，表示告警图片中支持中文显示，如下所示：

   ![](images/cluster_monitor/3-5.png)
4. 要在邮箱告警中显示图片，就要安装 [grafana-images/cluster\_monitor-renderer](https://grafana.com/grafana/plugins/grafana-image-renderer/)。然后修改 `[unified_alerting.screenshots]` 选项下的 `capture`，将其赋值为 true，表示在告警邮件中加入图片，如下所示：
   ![](images/cluster_monitor/3-6.png)
...
![](images/cluster_monitor/4-2.png)  
...
![](images/cluster_monitor/4-4.png)  
...
`$B>80`表示若表达式B返回的结果大于80，返回1，否则返回0。引用其他表达式或者查询的结果时，要使用一个'B'表示。
...
其余的为非必选选项，可以删除。`Summary` 为可使用模板变量，使得在告警消息中显示出具体的设备名称和值，如 `{{$labels.job}} 服务器cpu使用率为：{{values.B}}$`，若 A 服务器的 CPU 使用率为 90.14，则使用该语句后，在告警消息中会显示出：A 服务器 CPU 使用率为：90.14。更多支持的模板变量见[Grafana模板变量](https://grafana.com/docs/grafana/latest/alerting/fundamentals/alert-rules/message-templating/)。
...
此外，Grafana 可以根据告警标签设置更加复杂的发送策略，比如不同的告警发给不同的组员、设置特定时间段屏蔽告警等，详情请参考[附录-grafana告警的进阶使用](#grafana-%E5%91%8A%E8%AD%A6%E7%9A%84%E8%BF%9B%E9%98%B6%E4%BD%BF%E7%94%A8)。
...
![](images/cluster_monitor/4-18.png)  
...
![](images/cluster_monitor/5-20.png)  

点击`添加机器人`,出现如下所示页面：

![](images/cluster_monitor/5-21.png)  

选择`自定义`，跳转到如下页面：

![](images/cluster_monitor/5-22.png)  

`安全设置`选择IP地址段，输入你的 Grafana 所在的公网 IP，点击确定后出现如下所示界面：

![](images/cluster_monitor/5-23.png)  
...
![](images/cluster_monitor/5-25.png)  

选择 `ActionCard`，告警消息将以消息的样式发到钉钉群里，如下所示：

![](images/cluster_monitor/5-26.png)  
...
![](images/cluster_monitor/5-30.png)  

进入如下所示页面：

![](images/cluster_monitor/5-31.png)  

上传应用图片，给应用取一个名字，并选择应用可见范围（必须设置），创建完成后，进入如下所示页面：

![](images/cluster_monitor/5-32.png)  

先将 AgentId 和 Secret 记录下来，然后再配置企业可信 IP，如下所示：

![](images/cluster_monitor/5-33.png)  

点击：

![](images/cluster_monitor/5-34.png)  

填入 Prometheus 和 Aertmanager 部署的服务器的公网 IP，然后点击`通讯录`，如下所示：

![](images/cluster_monitor/5-35.png)  

获取到你要接收信息的部门的部门 ID，并记录。最后点击`我的企业`，如下所示：

![](images/cluster_monitor/5-36.png)  
