---
source_url: https://docs.dolphindb.cn/zh/progr/sql/roll.html
fetched_at: 2026-05-19T09:03:32Z
category: progr
title: roll
sha1: 83031016360dc055617c63ef891f0196508f5673
---
...
`roll` 是一个哑函数，用于在 CONTEXT BY 查询中定义滑动窗口的“步长”，而窗口长度由 SELECT
子句中的函数决定。

标准 CONTEXT BY 子句，或 GROUP BY 搭配分析函数时，窗口滑动步长固定为 1 行。GROUP BY 配合哑函数
`interval` 可以指定任意时间步长，但要求所有窗口长度相同。使用 `roll`
可以突破这些限制，自由定义窗口步长和长度。
...
- 当 *stepType*='window' 时，窗口长度由自定义窗口（*step* 参数）确定。
- *stepType* ≠ 'window'：`roll` 只定义步长，不确定窗口长度。窗口长度则由 SELECT
  ...
  - 内置 m 系列滑动函数：
    ...
    - 当  *stepType* = 'time' 时，若函数中 *window* 是 DURATION
      类型时，窗口长度 = *window*；若 *window* 是整数时，窗口长度是步长的
      *window* 倍。
    - 当 *stepType* = 'row' 时，函数中 *window* 必须是整数，窗口长度等于
      *window*。
    - 当 *stepType* = 'volume' / 'cumvol' 时，函数中 *window*
...
**X** 用于滚动的列，其数据类型取决于 *stepType*，例如可以是时间列或交易量列。
...
- 若 *stepType* 为 'window'，*X* 的数据类型必须与 *step* 中定义的窗口边界类型一致。
- 若 *stepType* 为 'row'，对 *X* 的类型不做要求。

**step** 窗口滑动步长，可以为：

- DURATION 类型标量，表示时间间隔，支持如下时间单位：M, w, d, H, m, s, ms, us, ns。
- 当 *stepType*='window' 时，*step* 是长度为 2
  的元组，用于指定自定义窗口。元组中的两个元素分别表示窗口的起始值和结束值（结束值必须大于起始值），其数据类型需与 X
  列保持一致，并按升序排列。默认左闭右开，可通过 *closed* 参数调整。
- 当 *stepType* = 'volume' 或 'cumvol' 时，*step* 是整型，表示窗口滑动步长。
...
- 当 *stepType* ≠ 'window' 时，向量的每个元素表示当前窗口中 X 列的最后一个值（窗口结束标识）。
- 当 *stepType* = 'window' 时，每个元素表示用户定义的窗口标签（*label*）。
