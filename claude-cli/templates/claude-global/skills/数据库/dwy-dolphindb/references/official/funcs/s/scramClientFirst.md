---
source_url: https://docs.dolphindb.cn/zh/funcs/s/scramClientFirst.html
fetched_at: 2026-05-19T09:38:04Z
category: funcs
title: scramClientFirst
sha1: fb8a1b43b8291d332deae8b84d313d7a9d541581
---
...
`scramClientFirst` 用于 SCRAM（Salted Challenge Response Authentication
Mechanism）认证过程的第一阶段。通过该函数可获取服务器端的认证信息。
...
服务端收到信息后，通过`scramClientFirst`函数生成用于加密的哈希盐值、密码哈希的迭代次数，并生成 nonce
...
服务端收到请求后，通过`scramClientFinal`对拼接后的 nonce
...
**cnonce** 是用户在客户端生成的 [nonce](https://en.wikipedia.org/wiki/Cryptographic_nonce)，必须是 16 字节的一次性随机数，使用 base64 编码。
...
- salt：base64 编码的哈希盐值
- iterCount：密码哈希的迭代次数
- combined\_nonce：客户端 nonce 和服务器端 nonce 的 base64 编码组合

## 例子

以下脚本是一个完整的示例，展示了如何在 Python 客户端完成用户（”user01”）SCRAM 模式登录，其中服务端的操作通过 DolphinDB Python
API 的相关接口实现（详见 [Python API 手册](https://docs.dolphindb.cn/zh/pydoc/BasicOperations/Session/Constructor.html)）。
