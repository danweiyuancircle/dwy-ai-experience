...
新项目集成 EUI 时，读取 `references/eui-integration-guide.md`，包含完整的安装、Vite 配置、样式引入、组件注册、主题切换等步骤。

## 设计风格规范

- **中后台页面**：读取 `references/eui-design-guide.md`，包含颜色系统、布局规范、排版规范、组件使用规范、交互规范等。
- **落地页/营销页**：读取 `references/eui-landing-design-guide.md`，包含 Hero、功能展示、社会证明、定价、FAQ 等 Section 模板和动画规范。
