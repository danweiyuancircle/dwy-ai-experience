---
name: dwy-sdk-spec
description: "SDK 接口规范与发布安全检查。检查 SDK 接口注释是否泄露后端实现（内网域名、数据库表名、Redis key、内部服务名、员工邮箱、JIRA 链接、SQL 语句），检查商业版 Python SDK 是否用 Cython 编译成 .so 隐藏源码，检查商业版 JS SDK 是否用 tsup/rollup minify + files 白名单，检查 PyPI/npm 发布产物是否含 .py 源文件 / .map source map / .env / .npmrc / tsbuildinfo / __pycache__ 等隐藏泄露文件。跨项目复用：首次进入新项目用 AskUserQuestion 问商业版还是开源版，缓存到 ~/.config/dwy-sdk-spec/projects.json，下次自动按规则检查。涉及以下任何场景必须使用此 skill：用户说「检查 SDK」「SDK 发版前检查」「SDK 注释脱敏」「SDK 接口规范」「准备发 PyPI」「准备发 npm」「商业版 SDK 打包」「Cython 编译 .so」「打包前体检」「.gitignore 漏了什么」「npm pack 看看包里有什么」「发布前安全检查」「SDK 源码泄露检查」「重设 SDK 版本类型」。"
---
...
路径：`~/.config/dwy-sdk-spec/projects.json`
...
- `languages` / `sdk_paths` 自动探测
- key 是项目**绝对路径**（`pwd` 取），跨机器迁移自动失效
...
- 输出非 `null` → 已配置，提取 `edition` / `languages` / `sdk_paths`，跳到对应流程
- 输出 `null` → 走「首次配置」
...
| 商业版但项目特殊不用 Cython（如纯接口包） | 跟用户确认；要长期跳过 Python 商业打包检查 → `bash scripts/init_config.sh reset` 重选「开源版」，或单次跳过流程 B |
...
| 缓存文件损坏（jq 解析失败） | 提示用户备份原文件后删除 `~/.config/dwy-sdk-spec/projects.json`，重新走首次配置 |
...
4. 缓存文件 `~/.config/dwy-sdk-spec/projects.json` 是本机配置，**不**随 skill 同步出去

## 安全约束

- skill 不存任何 token / 凭证 / 私钥
- skill 不动 `~/.pypirc` / `~/.npmrc`
- 脚本只做本地读 + 本地写 `~/.config/dwy-sdk-spec/`
