...
适用于 **Android 原生项目**(Java + Kotlin 混编,XML 布局)。本规范**重点是代码注释**,兼带最小必要的命名 / 简约 / 反模式约束。
...
- **用户可见文案必须走 `strings.xml`**,便于国际化与统一审校
...
- 全部使用**中文**,**禁止**中英混用
- 公开 API 注释**必须**有,**禁止**简单的"获取 / 设置 / 处理"敷衍式注释
- 代码改动后**必须**同步更新注释,留下过时注释 = 违规

### 2.1 文件 / 类级注释(强制)

每个公开类必须有类级 Javadoc / KDoc,放在类声明上方,说明:
- 该类的**职责**
- 与谁配合工作(被谁调用、依赖谁)
...
所有 `public` / Kotlin 默认可见性的方法 / 函数(含顶层函数)必须有 Javadoc / KDoc,包含:
- 一句话功能描述
...
| # | 检查项 | 违规即 STOP |
|---|--------|------------|
| 1 | 公开类有类级 Javadoc / KDoc(中文),说明职责与协作关系 | ✓ |
| 2 | 公开方法 / 函数有 Javadoc / KDoc,含 `@param` / `@return` / `@throws` | ✓ |
