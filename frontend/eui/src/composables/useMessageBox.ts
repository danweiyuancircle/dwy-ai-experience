...
export function useMessageBox() {
  function open(options: MessageBoxOptions): Promise<'confirm' | 'cancel'> {
    return new Promise((resolve) => {
      ...
      box.innerHTML = `
        ${options.title ? `<div class="font-semibold text-lg mb-2">${options.title}</div>` : ''}
        <div class="text-muted-foreground text-sm mb-6">${options.message}</div>
        <div class="flex justify-end gap-2">
          ${options.showCancelButton !== false ? `<button class="eui-msgbox-cancel inline-flex items-center justify-center rounded-md border px-4 py-2 text-sm font-medium hover:bg-accent">${options.cancelButtonText ?? '取消'}</button>` : ''}
          <button class="eui-msgbox-confirm inline-flex items-center justify-center rounded-md bg-primary text-primary-foreground px-4 py-2 text-sm font-medium hover:bg-primary/90">${options.confirmButtonText ?? '确定'}</button>
        </div>
      `
