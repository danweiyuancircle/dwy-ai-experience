...
- 技术栈/场景包下的子条目随父包显示为已勾选，不再画成删除线空框。逐条取消仍在下一步预勾清单里。
...
- 技术栈包与场景包都会在包名下列出子条目；选包后统一进入预勾清单，可逐条取消。
...
- `dwy` 菜单只留两项：同步全局 skills、同步项目配置。升级只走 `dwy upgrade`。
- 同步项目配置里分「按场景和技术栈」与「一个一个选」。按包后直接列出已预勾子条目，可取消。
