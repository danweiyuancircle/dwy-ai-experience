...
**在编写任何后端代码前，以及调用任何 dwyeapi 提供的 API 之前，必须先用 `Skill` 工具加载 `dwy-eapi`。** skill 会按消费方**实际安装版本**选 `v0`/`v1` 文档线；不确定时先 clone `dwyeapi@x.y.z`，拿不到再读虚拟环境里的安装包。本文件**只规定约束与边界**，**不固化**模块路径、类名、函数签名。
