...
- Android 端默认按全面屏、刘海屏、挖孔屏适配实现,不要使用固定上下左右边距硬编码内容区。
- 优先使用 `WindowInsets`、系统栏 inset、约束布局和响应式容器处理状态栏、导航栏、手势区域占位。
- 顶部标题栏、底部操作栏、输入区、悬浮按钮必须显式考虑状态栏、导航栏和手势返回区域,避免被遮挡或贴边误触。
