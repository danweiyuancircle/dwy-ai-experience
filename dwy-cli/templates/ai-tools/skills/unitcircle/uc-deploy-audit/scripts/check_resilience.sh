...
#
# 检查范围:
#   B 类: 系统服务开机自启 (sshd / nginx / docker / postgres / redis / fail2ban / 自定义)
...
# 默认必检清单
DEFAULT_SERVICES=(sshd ssh nginx docker docker.socket postgresql postgres redis redis-server fail2ban supervisor crond cron)
