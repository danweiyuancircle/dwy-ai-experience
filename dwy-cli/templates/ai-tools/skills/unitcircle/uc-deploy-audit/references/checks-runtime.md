...
| **daemon `registry-mirrors`** | 已配置至少 1 个 mirror(按服务器所在地区选择) | medium(未配置) / info |
| **运行容器使用境外 registry**(`gcr.io` `ghcr.io` `k8s.gcr.io` `quay.io` `mcr.microsoft.com` `nvcr.io` `docker.elastic.co`) | `registry-mirrors` 不对它们生效, 拉取加速需自行解决 | info |
...
> 脚本只输出 raw（硬件规格 + 当前容器 mem_limit + Postgres/Redis 启动参数 + compose 资源声明）；主 Claude 用下表生成 "推荐 vs 当前" 对比报告。
...
- 容器 `mem_limit` 合计 ≤ 宿主总内存的 **65%**（预留 OS / 全局 nginx / 监控 agent 等）
