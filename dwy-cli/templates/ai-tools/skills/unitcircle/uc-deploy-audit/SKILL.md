...
# 部署后基础环境巡检 (uc-deploy-audit)

对**已部署上线**的服务器执行只读式安全合规审计，逐项核对基础环境配置，输出分级报告。

**本工具边界：** 白盒只读配置审计——仅 SSH 登录读取配置文件与运行状态，不做主动扫描、漏洞利用、暴力破解等渗透行为。如需渗透测试请使用专用渗透工具。
...
**主 Claude 直接用 Bash 调用 `run_all.sh`,12 路并行跑完全部检查类目**,把原始输出落到 `/tmp/uc_audit_<host>.txt`,再由主 Claude `Read` 文件后做分析。
...
bash {scripts}/run_all.sh <ssh_alias>           > /tmp/uc_audit_<tag>.txt 2>&1

# 用户提供显式参数
bash {scripts}/run_all.sh <user>@<host> -p <port> -i <key>  > /tmp/uc_audit_<tag>.txt 2>&1
```
...
**输出文件命名规则**：`/tmp/uc_audit_<host_or_alias>.txt`，便于用户回看 + 多次审计互不覆盖。
...
```bash
bash {scripts}/run_all.sh <target> [ssh_opts...] --serial > /tmp/uc_audit_<tag>.txt 2>&1
```
...
→ 详细检查项、期望值、严重级见 `references/checks-runtime.md` §「4.6 Docker 安全」
...
读取 5 类凭证源（.env / docker env / Redis requirepass / Postgres POSTGRES_PASSWORD / SSH 私钥），输出强度派生指标 `len / classes / strength / reason`，**强制脱敏不输出明文**。
...
2. **拿"实际数据"**:用 Read 工具读 `/tmp/uc_audit_<host>.txt`(run_all.sh 输出)
...
| 6 | ~/.ssh/id_ed25519 | type+bits | high | ✅ | ED25519 perm=600 |
| 7 | ~/.ssh/id_rsa | type+bits | medium | ❌ | RSA-2048 perm=600(建议升 ed25519) |
...
- 镜像版本不固定 (4.6) → 参考 Docker Hub 选稳定 patch 版本固定
- daemon 未配 registry-mirrors (4.6) → 按服务器所在地区配置合适的 registry-mirrors
