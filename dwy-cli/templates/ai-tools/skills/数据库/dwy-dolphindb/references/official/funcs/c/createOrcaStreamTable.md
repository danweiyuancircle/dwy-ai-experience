---
source_url: https://docs.dolphindb.cn/zh/funcs/c/createOrcaStreamTable.html
fetched_at: 2026-05-19T09:16:43Z
category: funcs
title: createOrcaStreamTable
sha1: 569fb1124166e200baa6c92630eb2e99742326e7
---
...
`createOrcaStreamTable(name, colNames, colTypes, [asyncWrite=true],
...
创建一个持久化共享流数据表。详情参考 [enableTableShareAndPersistence](https://docs.dolphindb.cn/zh/funcs/e/enableTableShareAndPersistence.html)。

与 `StreamGraph::source` 的区别在于，`createOrcaStreamTable`
无需依赖流图。

注：

- 调用该函数前，必须先启用 Orca 功能，即在配置文件 cluster.cfg 或 controller.cfg 中配置
  `enableORCA=true`）。
- 在集群环境下，该函数只能在计算组中的计算节点上运行。
