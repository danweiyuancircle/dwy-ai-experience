---
source_url: https://docs.dolphindb.cn/zh/rn/server/3_00_3.html
fetched_at: 2026-05-19T09:52:01Z
category: rn
title: 3.00.3
sha1: 0adf7913afe1cbb00de376b6031323fa6ec8dd2f
---
...
同时建议阅读[兼容性说明](../compact_report_3_00_3.html)。
...
- 新增函数 `encryptModule` ，用于对 dos 文件进行加密。（**3.00.3.5**）
- 新增向量相似度与距离计算函数：（**3.00.3.5**）
...
- 支持对 API 传入的字符串，根据字段类型自动转换为 DOUBLE、FLOAT 或 DECIMAL，并插入表中。（**3.00.3.5**）
- 优化持久化流表在重新加载过程中的内存占用。（**3.00.3.5**）
- 流订阅根 job ID 改为随机 UUID。（**3.00.3.5**）
- 支持在 `getAclAuditLog`、`getAuditLog` 的返回结果中展示节点名称（node alias）。（**3.00.3.5**）
- 优化过期 chunk 版本回收机制。（**3.00.3.5**）
- 优化 license server 心跳异常后的关机逻辑，并新增 license server 状态信息。（**3.00.3.5**）
  - 优化未正常收到 license server 服务器回复后的关机机制，将自动关机等待时间从 2 天延长至 14 天。
  - 新增函数 `getLicenseServerStatus()`，用于获取 license server 状态。
- 优化 orderbookEngine 在盘口倒挂场景下的 WARNING 日志打印策略。（**3.00.3.5**）
- 支持对 API 传入的字符串，根据字段类型自动转换为 DOUBLE、FLOAT 或 DECIMAL，并插入表中。（**3.00.3.5**）
- 优化持久化流表在重新加载过程中的内存占用。（**3.00.3.5**）
- 流订阅根 job ID 改为随机 UUID。（**3.00.3.5**）
- 支持在 `getAclAuditLog`、`getAuditLog` 的返回结果中展示节点名称（node alias）。（**3.00.3.5**）
...
- 修复将 VOID 类型列（如通过 SELECT null 生成）追加到 SYMBOL 类型列时报错的问题。（**3.00.3.5**）
- 修复 SQL 语句计算表达式中减法操作符前后存在空格时，系统将“减 1”错误解析为“负 1”并报错的问题。（**3.00.3.5**）
- 修复在 TSDB/IOTDB 引擎的分区表查询中，`select` 使用嵌套三元运算符 `?:` 时无法正确解析所需列名的问题。（**3.00.3.5**）
- 修复部分条件下直接对分布式库表执行 pivot by 时结果可能出现空值的问题。（**3.00.3.5**）
- 将 SYMBOL 类型字段设置为 TSDB 引擎的 sortKey，导致节点偶发崩溃。（**3.00.3.5**）
- 修复 TSDB 引擎查询命中 cache engine 缓冲区时，因查询条件里 SYMBOL 列的过滤条件不是 LITERAL 类型导致报错的问题。（**3.00.3.5**）
- `dropTable` 与数据写入同时执行导致集群卡住。（**3.00.3.5**）
- 修复使用 `insert into 表(列名)` 插入部分列时报错信息不准确的问题，在不支持该功能的版本中会提示暂不支持并建议升级到最新版本。（**3.00.3.5**）
- 修复查询横截面引擎时的并发问题，避免在读写过程中因数据（如表行数）不一致导致 SQL 查询报错。（**3.00.3.5**）
- 修复当 `createWindowJoinEngine` 的 *window* 参数设置为 0:0 后，分组内数据量超过 *garbageSize* 限制时部分时间窗口结果不输出的问题。（**3.00.3.5**）
- 修复表连接引擎在计算数组向量字段时，结果未正确更新的问题。（**3.00.3.5**）
- 修复 `getStreamTables` 的 totalRows 字段溢出的问题。（**3.00.3.5**）
- 修复高可用集群在执行 `moveReplicas` 时，因缓存句柄未失效导致查询结果偶发不一致的问题。（**3.00.3.5**）
- 修复在 `lanCluster=true` 的高可用集群中，执行 stopController.sh 脚本后 controller 进程无法退出的问题。（**3.00.3.5**）
- 修复 `getUserTableAccessRecords` 在查询范围跨越两个及以上 job 文件时报错的问题。（**3.00.3.5**）
- 修复在存在计算组的集群中执行 DDL 操作时，触发不符合预期的权限报错的问题。（**3.00.3.5**）
- 修复偶发 jobcheckpoint 文件描述符泄露的问题。（**3.00.3.5**）
- 修复从 2.00.16.X/2.00.17.X 升级至 3.00.3.X/3.00.4.X 时的兼容性问题。（**3.00.3.5**）
- 修复在 OOM 场景下 `getSessionMemoryStat` 的 OLAPTablet 内存统计错误，导致缓存清理逻辑失效的问题。（**3.00.3.5**）
- 修复 `saveText` 导出 CSV 时对 SYMBOL 类型字段中引号处理出错的问题。（**3.00.3.5**）
- 修复整数类型解析的如下问题：（**3.00.3.5**）
  - 修复通过 `loadText` 导入包含科学计数法的 SHORT、INT、LONG 类型数据时，解析结果不符合预期的问题。
  - 修复负数 LONG 类型数据末尾包含空格时，负号可能丢失的问题。
- 修复使用 `xdb` 连接后，远程执行脚本偶发报错的问题。（**3.00.3.5**）
- 修复 `linearTimeTrend` 在窗口大于 1024 时结果计算错误的问题。（**3.00.3.5**）
- 修复 `percentile` 计算可能触发段错误的问题。（**3.00.3.5**）
- 修复 `bar` 函数在处理 TIME 等时间类型时可能生成超过类型上限的异常值的问题。（**3.00.3.5**）
- 修复在 *triggerType=independent* 模式下，OrderBookSnapshotEngine 输入数据的 SecurityID 为 NULL 且触发快照输出时可能导致宕机的问题。（**3.00.3.5**）
- 修复将 VOID 类型列（如通过 SELECT null 生成）追加到 SYMBOL 类型列时报错的问题。（**3.00.3.5**）
- 修复 SQL 语句计算表达式中减法操作符前后存在空格时，系统将“减 1”错误解析为“负 1”并报错的问题。（**3.00.3.5**）
- 修复在 TSDB/IOTDB 引擎的分区表查询中，`select` 使用嵌套三元运算符 `?:` 时无法正确解析所需列名的问题。（**3.00.3.5**）
- 修复部分条件下直接对分布式库表执行 pivot by 时结果可能出现空值的问题。（**3.00.3.5**）
