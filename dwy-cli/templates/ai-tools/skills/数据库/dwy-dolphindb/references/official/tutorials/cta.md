---
source_url: https://docs.dolphindb.cn/zh/tutorials/cta.html
fetched_at: 2026-05-19T09:47:20Z
category: tutorials
title: 外汇中高频 CTA 风控策略回测案例
sha1: 41deff99b91590c13d267bf22d1647a879e934f8
---
...
回测引擎采用了响应式状态引擎。这一引擎能够实现流批统一计算，并有效处理带有状态的高频因子，具体也可以参考[响应式状态引擎用户手册](../funcs/c/createReactiveStateEngine.html)定义相应的指标。以下是定义技术指标 RSI 和 Bollinger Bands 的代码示例：
...
创建回测引擎之后，可以通过以下方式执行回测。backtestdata\_ 为相应的分钟频率行情数据，行情数据字段和类型说明参考 [回测插件的接口文档](../plugins/backtest.html)。
...
回测运行结束之后，可以通过相应的接口获取每日持仓、每日权益、收益概述、成交明细和策略中用户自定义的逻辑上下文。回测插件提供的完整回测结果接口可以参阅 [回测插件的接口文档](../plugins/backtest.html)。下图为本例获得的交易明细结果：
