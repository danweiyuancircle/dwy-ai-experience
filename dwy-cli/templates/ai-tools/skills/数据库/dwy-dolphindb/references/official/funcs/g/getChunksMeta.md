---
source_url: https://docs.dolphindb.cn/zh/funcs/g/getChunksMeta.html
fetched_at: 2026-05-19T09:23:00Z
category: funcs
title: getChunksMeta
sha1: 155e4bdb56dd97d04e0bdc3e17a53416cb12da8d
---
...
查询当前节点上指定数据库 chunk 的元数据。若不指定 *chunkPath*，返回当前节点上所有数据库 chunk 的元数据。

## 参数

**chunkPath** 是一个或多个 chunk 的 DFS 路径，支持使用通配符 \*，% 以及？。

- \* 用于匹配任意数量的任意字符。比如 "/testDB/\*" 表示匹配以 /testDB/ 开头的所有路径。
- % 用于匹配 0个、1个或多个任意字符。比如 "/testDB/%/8" 表示匹配以 /testDB/ 开头、以 /8结尾的所有路径。
- ? 用于匹配1个任意字符。比如 "/testDB/1?/8" 表示匹配 /testDB/10/8、/testDB/11/8 等路径。

**top** 是一个正整数，指定结果中返回的最大 chunk 数。默认值为1024。若设置 *top*= -1，则不限制返回的 chunk
的数量。
...
- flag：删除标志。若 flag=0，表示此 chunk 数据可以正常被查询和访问。若 flag=1，表示此 chunk
  数据已在逻辑上被标记为删除，不可被查询，但仍然占用磁盘空间。
  - 使用 delete 语句删除数据时不会删除对应的 chunk，因此返回结果中对应 chunk 的 flag 依然为 0。
  - 执行 [dropPartition](../d/dropPartition.html)、[dropDatabase](../d/dropDatabase.html)、[dropTable](../d/dropTable.html) 函数或 [drop](../../progr/sql/drop.html) 语句时，会将对应 chunk 的 flag
    会设为 1。但 flag = 1 且删除事务已完成时，则不会返回此 chunk 的元数据，因此通常看不到 flag=1 的结果。要看到
    flag=1 的 chunk，只能在删除事务未结束时，执行 `getChunksMeta`。
...
**相关函数**：[getTabletsMeta](getTabletsMeta.html)
