---
source_url: https://docs.dolphindb.cn/zh/tutorials/oom_settlement_2.html
fetched_at: 2026-05-19T09:49:38Z
category: tutorials
title: Out of Memory
sha1: 3b74eff07952f32b1bd8dad7d3dcd79b65396cc3
---
...
- **合理配置 *maxMemSize*** ：
  ...
  - 在配置 *maxMemSize* 时，还需考虑并发执行参数（*maxBatchJobWorker*）对内存使用的影响。应结合单任务的内存占用评估系统的最大并发能力，并合理设置相关参数，以避免在高并发场景下发生 OOM。
- **对自定义插件内存严格管理**
...
- **避免大事务写入**：通过[append！](https://docs.dolphindb.cn/zh/funcs/a/append!.html?hl=append)或[tableInsert](https://docs.dolphindb.cn/zh/funcs/t/tableInsert.html?hl=tableinsert)批量写入较大数据量时，会长时间占用内存，导致内存无法释放。
