---
source_url: https://docs.dolphindb.cn/zh/tutorials/backtesting_using_MatchingEngineSimulator.html
fetched_at: 2026-05-19T09:47:29Z
category: tutorials
title: 在 C++ 使用模拟撮合引擎插件实现自定义的高性能回测
sha1: b6086aa0f5692327c19b637e47a2d162564a4e24
---
...
第一种实现为展开的参数列表，第二种实现将第一种实现的所有参数放在一个数组里。该接口的参数要求与[模拟撮合引擎插件](../plugins/matchingEngineSimulator/mes.html)的
...
在 Swordfish 中可以使用 [xdb](../funcs/x/xdb.html) 方法远程连接
DolphinDB 节点，然后使用 [remoteRun](../funcs/r/remoteRun.html)
...
在 Swordfish 中本地加载和使用模拟撮合引擎插件，可以通过 [loadPlugin](../funcs/l/loadPlugin.html) 方法加载并获得其接口的函数指针。为了实现便捷的插件加载和调用相关接口操作，我们封装了 loadPlugin
...
在 Swordfish 中可以使用 [subscribeTable](../funcs/s/subscribeTable.html) 方法订阅远程 DolphinDB 的流表，该方法的一个关键参数是 handler，
...
在 C++ API 可以使用 [DBConnection](../api/cpp/create_dbconn_obj.html) 类远程连接 DolphinDB 和执行脚本。具体接口使用说明请参考 C++ API 官方文档。

#### 4.2.2 插件加载和使用

在 C++ API 实现时，模拟撮合引擎插件在 DolphinDB 远程加载和使用，可以远程执行 [loadPlugin](../funcs/l/loadPlugin.html) 方法加载插件到
DolphinDB，然后封装远程连接类的 [run](../funcs/r/run.html)
接口执行模拟撮合引擎的接口脚本，实现模拟撮合引擎插件类
MatchingEngineSimulatorWrapper，实现下单和撤单等主动调用接口。具体接口使用说明请参考 C++ API 官方文档。

#### 4.2.3 流数据订阅、解析和回调

在 C++ API 可以使用 [ThreadedClient](../api/cpp/threadedclient.html) 类订阅远程 DolphinDB 的流表，该类提供的 [subscribe](https://docs.dolphindb.cn/zh/cppdoc/threadedclient.html#threadedclient__section_njy_shm_x1c) 方法可以直接以 C++
