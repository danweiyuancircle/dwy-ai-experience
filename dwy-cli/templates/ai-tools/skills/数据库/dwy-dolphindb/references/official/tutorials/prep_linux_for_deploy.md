---
source_url: https://docs.dolphindb.cn/zh/tutorials/prep_linux_for_deploy.html
fetched_at: 2026-05-19T08:59:41Z
category: tutorials
title: Linux 系统部署准备
sha1: ab1c5a67ac7d9cd75ca1c0bf35b34a3eb20b487a
---
...
在 Linux 系统使用 DolphinDB 要求内核版本为 Linux 2.6.19 或以上。对于 DolphinDB 2.00.16/3.00.3 以下版本，推荐使用 **CentOS 7 稳定版**；对于 2.00.16/3.00.3 或以上版本，推荐使用 **Rocky Linux 9 稳定版**。
...
**注意：应确保 x86 架构 CPU 支持 avx 指令集。** 可通过 lscpu | grep avx 查看当前环境的 CPU 是否支持该指令集。返回输出中如果包含 avx 指令集，则说明当前环境 CPU 支持 avx 指令集。

若使用 Ubuntu 系统，注意以下几点：

- 需要在系统安装时在 Storage configuration 中选择 /home 目录的格式为 xfs，以方便后续将 /home 目录格式化为 xfs 文件系统；
- Ubuntu 系统使用 ufw 防火墙，关闭防火墙命令为 `sudo ufw disable`，其他命令请参考 `man ufw`；
- Ubuntu 系统使用 apport 处理 core dump，若需要自定义 core dump，需要禁用 apport，命令为 `sudo systemctl disable apport.service`
...
硬盘 IO 性能提升对于大数据处理非常重要，推荐硬件配置见 [推荐硬件配置](#%E6%8E%A8%E8%8D%90%E7%A1%AC%E4%BB%B6%E9%85%8D%E7%BD%AE)。
...
2. 使用 `mkfs.xfs` 命令格式化新硬盘为 xfs 文件系统类型：
...
5. 设置开机自动挂载
   ...
   **注意**：这里使用了 UUID 来标识新硬盘，避免了磁盘更换位置导致识别出错。
