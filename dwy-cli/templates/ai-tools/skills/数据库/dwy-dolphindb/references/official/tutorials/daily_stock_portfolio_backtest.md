---
source_url: https://docs.dolphindb.cn/zh/tutorials/daily_stock_portfolio_backtest.html
fetched_at: 2026-05-19T09:47:15Z
category: tutorials
title: 股票组合管理日频回测案例
sha1: 1e7c077630384ffba9a1bcbf94dbd6c7f76edf7a
---
...
为相应的日频行情数据，行情数据字段和类型说明可参考[回测插件的接口文档](../plugins/backtest.html)。
...
回测运行结束之后，可以通过相应的接口获取每日持仓、每日权益、收益概述、成交明细和策略中用户自定义的逻辑上下文。回测插件提供的完整回测结果接口可以参阅[回测插件的接口说明文档](../plugins/backtest.html)。下图为本例获得的交易明细结果：
