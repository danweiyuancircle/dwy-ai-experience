...
分类目录必须和同步 Tab 的包名对齐：技术栈用 `通用` / `Vue` / `Python` / `Docker` 等，场景用 `产品0到1` / `自媒体` / `发布发版` / `安全` / `内容创作` / `DolphinDB`。不要再建 `基础库` / `脚手架` / `元工具` 这种与包不对齐的目录。新文件明显不属于任何现有包时，**主动建议新建分类（同时要在 `sync-packs.js` 加包）**。
