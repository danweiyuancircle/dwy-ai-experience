...
5. 产出：`.dwy/prod/[项目]/03-设计与架构/prototypes/wireframe/`
...
6. 产出：`.dwy/prod/[项目]/03-设计与架构/prototypes/hifi/`
7. **门控**：产出后汇报，问「高保真平面图是否通过，可进架构？」。通过后回写 `confirmed.prototype`。

## 产出契约（硬约束）
- 两轮目录分开：`prototypes/wireframe/` 与 `prototypes/hifi/`，各含每页 `<页面>.html` + `index.html` 总览
